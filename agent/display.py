...
def get_cute_tool_message(
    ...
    if tool_name == "web_search":
        return _wrap(f"┊ 🔍 {_trunc(args.get('query', ''), 52)}  {dur}")
    if tool_name == "web_extract":
        ...
        if urls:
            ...
            return _wrap(f"┊ 📄 {_trunc(domain, 44)}{extra}  {dur}")
        return _wrap(f"┊ 📄 pages  {dur}")
    if tool_name == "web_crawl":
        url = args.get("url", "")
        domain = url.replace("https://", "").replace("http://", "").split("/")[0]
        return _wrap(f"┊ 🕸️  {_trunc(domain, 46)}  {dur}")
    if tool_name == "terminal":
        return _wrap(f"┊ 💻 {_trunc(args.get('command', ''), 52)}  {dur}")
    if tool_name == "process":
        action = args.get("action", "?")
        sid = args.get("session_id", "")[:12]
        labels = {"list": "ls", "poll": f"poll {sid}", "log": f"log {sid}",
                  "wait": f"wait {sid}", "kill": f"kill {sid}", "write": f"write {sid}", "submit": f"submit {sid}"}
        return _wrap(f"┊ ⚙️  {labels.get(action, f'{action} {sid}')}  {dur}")
    if tool_name == "read_file":
        return _wrap(f"┊ 📖 {_path(args.get('path', ''))}  {dur}")
    if tool_name == "write_file":
        return _wrap(f"┊ ✍️  {_path(args.get('path', ''))}  {dur}")
    if tool_name == "patch":
        return _wrap(f"┊ 🔧 {_path(args.get('path', ''))}  {dur}")
    if tool_name == "search_files":
        pattern = _trunc(args.get("pattern", ""), 44)
    ...
    if tool_name == "browser_navigate":
        url = args.get("url", "")
        domain = url.replace("https://", "").replace("http://", "").split("/")[0]
        return _wrap(f"┊ 🌐 {_trunc(domain, 46)}  {dur}")
    if tool_name == "browser_snapshot":
        mode = "full" if args.get("full") else "compact"
        return _wrap(f"┊ 📸 {mode}  {dur}")
    if tool_name == "browser_click":
        return _wrap(f"┊ 👆 {args.get('ref', '?')}  {dur}")
    if tool_name == "browser_type":
        return _wrap(f"┊ ⌨️  \"{_trunc(args.get('text', ''), 44)}\"  {dur}")
    if tool_name == "browser_scroll":
        d = args.get("direction", "down")
        arrow = {"down": "↓", "up": "↑", "right": "→", "left": "←"}.get(d, "↓")
        return _wrap(f"┊ {arrow}  {dur}")
    if tool_name == "browser_back":
        return _wrap(f"┊ ◀️  {dur}")
    if tool_name == "browser_press":
        return _wrap(f"┊ ⌨️  {args.get('key', '?')}  {dur}")
    if tool_name == "browser_get_images":
        return _wrap(f"┊ 🖼️  {dur}")
    if tool_name == "browser_vision":
        return _wrap(f"┊ 👁️  {_trunc(args.get('question', ''), 46)}  {dur}")
    if tool_name == "todo":
        todos_arg = args.get("todos")
        merge = args.get("merge", False)
        if todos_arg is None:
            return _wrap(f"┊ 📋 reading  {dur}")
        elif merge:
            return _wrap(f"┊ 📋 update {len(todos_arg)}  {dur}")
        else:
            return _wrap(f"┊ 📋 {len(todos_arg)} task(s)  {dur}")
    if tool_name == "session_search":
        return _wrap(f"┊ 🗂️  \"{_trunc(args.get('query', ''), 44)}\"  {dur}")
    if tool_name == "memory":
        action = args.get("action", "?")
        target = args.get("target", "")
        if action == "add":
            return _wrap(f"┊ 🧠 +{target}: \"{_trunc(args.get('content', ''), 38)}\"  {dur}")
        elif action == "replace":
            return _wrap(f"┊ 🧠 ~{target}: \"{_trunc(args.get('old_text', ''), 30)}\"  {dur}")
        elif action == "remove":
            return _wrap(f"┊ 🧠 -{target}: \"{_trunc(args.get('old_text', ''), 30)}\"  {dur}")
        return _wrap(f"┊ 🧠 {action}  {dur}")
    if tool_name == "skills_list":
        return _wrap(f"┊ 📚 list {args.get('category', 'all')}  {dur}")
    if tool_name == "skill_view":
        return _wrap(f"┊ 📚 {_trunc(args.get('name', ''), 44)}  {dur}")
    if tool_name == "skill_manage":
        return _wrap(f"┊ 📚 {args.get('action', '?')} {_trunc(args.get('name', ''), 38)}  {dur}")
    if tool_name == "image_generate":
        return _wrap(f"┊ 🎨 {_trunc(args.get('prompt', ''), 46)}  {dur}")
    if tool_name == "text_to_speech":
        return _wrap(f"┊ 🔊 {_trunc(args.get('text', ''), 46)}  {dur}")
    if tool_name == "vision_analyze":
        return _wrap(f"┊ 👁️  {_trunc(args.get('question', ''), 46)}  {dur}")
    if tool_name == "mixture_of_agents":
        return _wrap(f"┊ 🌀 {_trunc(args.get('user_prompt', ''), 46)}  {dur}")
    if tool_name == "send_message":
        return _wrap(f"┊ 📨 {args.get('target', '?')}: \"{_trunc(args.get('message', ''), 38)}\"  {dur}")
    if tool_name == "cronjob":
        ...
        if action == "create":
            ...
            return _wrap(f"┊ ⏰ create {_trunc(label, 36)}  {dur}")
        if action == "list":
            return _wrap(f"┊ ⏰ list  {dur}")
        return _wrap(f"┊ ⏰ {action} {args.get('job_id', '')}  {dur}")
    if tool_name.startswith("rl_"):
        rl = {
        ...
        return _wrap(f"┊ 🧪 {rl.get(tool_name, tool_name.replace('rl_', ''))}  {dur}")
    if tool_name == "execute_code":
        if args.get("description"):
            preview = args["description"]
        else:
            code = args.get("code", "")
            preview = next((l.lstrip("# ") for l in code.strip().splitlines() if l.strip().startswith("#")), None)
            preview = preview or _trunc(code.strip().splitlines()[0] if code.strip() else "", 46)
        return _wrap(f"┊ 🐍 {_trunc(preview, 46)}  {dur}")
    if tool_name == "delegate_task":
        tasks = args.get("tasks")
        if tasks and isinstance(tasks, list):
            return _wrap(f"┊ 🔀 {len(tasks)} parallel tasks  {dur}")
        return _wrap(f"┊ 🔀 {_trunc(args.get('goal', ''), 46)}  {dur}")
    if tool_name == "async_delegate_task":
        label = args.get("label") or _trunc(args.get("prompt", ""), 40)
        return _wrap(f"┊ 🔀 async: {_trunc(label, 40)}  {dur}")
    if tool_name == "list_async_tasks":
        return _wrap(f"┊ 🔀 list async tasks  {dur}")
